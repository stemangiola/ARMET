...
#' @export
dirReg_test = function(obj, my_design, cov_to_test = NULL, which_cov_to_test = 2, names_groups = NULL){
	...
	stats = do.call("rbind", lapply(1:K, function(i){
		...
		pcap_human_readable = ifelse(
			pcap<0.01 & pcap>0,
			formatC(pcap, format = "e", digits = 2),
			round(pcap, 2)
		)
		...
		pip_human_readable =  ifelse(
			pip<0.01 & pip>0,
			formatC(pip, format = "e", digits = 2),
			round(pip, 2)
		)
