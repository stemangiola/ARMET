...
```{r, cache=TRUE}
# TEMPORARY
load("/stornext/Home/data/allstaff/m/mangiola.s/PhD/deconvolution/ARMET/docs/fit_MPI_level1_08042019.RData")
load("/stornext/Home/data/allstaff/m/mangiola.s/PhD/deconvolution/ARMET/docs/level_1_input_08042019.RData")
...
foreach(ct = fit_df %>% distinct(`Cell type category`) %>% filter(`Cell type category` != "house_keeping") %>% pull(1), .combine = bind_rows) %do% {
	fit_df %>%
...
{
	(.) %>%
		rename(`Cell type category` = `Cell type category`) %>%
    filter(`Cell type category` != "house_keeping" ) %>%
		...
		add_MDS_components(
			...
			value_column = "read count normalised",log_transform = T
		) %>% 
    distinct(sample, `Cell type category`, `Cell type formatted`, `PC 1`, `PC 2`, `Data base`) %>% 
    ggplot(aes(x=`PC 1`, y=`PC 2`, color=`Cell type category`, shape=`Data base`, label =`Cell type formatted`)) + geom_text() +
    geom_point() + my_theme
} %>%
...
{ ggplot((.), aes(y=`read count normalised bayes` + 1, x=`Cell type category` %>% substring(1, 2), fill=ct)) + 
	geom_boxplot() + 
