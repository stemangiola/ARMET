...
foreach(my_tot=seq(1, 1000, 10), .combine=bind_rows) %:%
	foreach(mp=c(0.01, 0.1, 1, 10), .combine=bind_rows) %do% {

	my_phi = c(mp, mp*5)
	my_mean = MCMCpack::rdirichlet(1,mu) * my_tot
  ...
  tibble(
  	...
  	bind_rows(
  		 tibble(
  		 	method=c("rough theory", "gamma theory"), 
  		 	mean=sum(data$means), 
  		 	phi=c(
  		 		 mean(my_phi) * (sum(data$means) ^ 2) / sum(data$means ^ 2),
  		 		sum_nb_through_gamma(my_mean, my_phi)[2]
  		 	)
  		 )
  	) %>% 
  	mutate(real_tot = my_tot, my_phi = mp)
